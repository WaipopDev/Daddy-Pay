...
import { LANGUAGE_DEFAULT_CODE } from '@/constants/languageSettings';
import { getClientLangCode, setClientLangCookie } from '@/lib/langCookie';
...
const AdminNavbar: React.FC<AdminNavbarProps> = ({ onMenuToggle, isMenuOpen }) => {
    ...
    const [activeLangCode, setActiveLangCode] = useState(LANGUAGE_DEFAULT_CODE)

    useEffect(() => {
        setActiveLangCode(getClientLangCode());
    }, []);
    ...
    const setActiveLanguage = async (code: string) => {
        ...
        if (normalized === activeLangCode) {
            return;
        }

        try {
            setClientLangCookie(normalized);

            const res = await axios.get(`/api/lang?langCode=${normalized}`);
            const translations =
                res.data && typeof res.data === 'object' && Object.keys(res.data).length > 0
                    ? (res.data as LanguageProp)
                    : (languageDefault as LanguageProp);

            dispatch(setLang(translations));
            setActiveLangCode(normalized);
        } catch (error) {
            console.error('Error switching language:', error);
            setClientLangCookie(normalized);
            dispatch(setLang(languageDefault as LanguageProp));
            setActiveLangCode(normalized);
        }
    }
                    ...
                    <div className="flex-col md:flex-row list-none items-center hidden md:flex">
                        <Dropdown className="nav-dropdown-lang mr-2">
                            <Dropdown.Toggle className="flex items-center w-full cursor-pointer px-3 py-2 lg:min-w-[180px] rounded-md">
                                ...
                                <p className="px-2 w-full text-left">
                                    {languages[activeLangCode] || activeLangCode}
                                </p>
                            </Dropdown.Toggle>
                            <Dropdown.Menu>
                                {_.map(languages, (lang, key) => (
                                    <Dropdown.Item
                                        ...
                                        className={`cursor-pointer ${activeLangCode === key.toUpperCase() ? 'bg-[#f0f0f0]' : ''}`}
                                    >
                                        {lang}
                                    </Dropdown.Item>
                                ))}
