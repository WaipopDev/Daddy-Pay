...
export async function DELETE(req: NextRequest) {
    try {
        ...
        const response = await axios.delete(`${process.env.API_URL}/api/v1/machine-info/${machineId}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
        ...
        if (response.status === 200) {
            return await createResponseWithHeaders(
                { message: 'Machine information deleted successfully' }, 
                response, 
                200
            );
        } else {
            return NextResponse.json({ message: 'Failed to delete machine information' }, { status: 401 });
        }
    } catch (error) {
        const err = error as AxiosError;
